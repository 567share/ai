...
import { Tool, BadgeType } from './types';
...
export default function App() {
  ...
  const fetchTools = useCallback(async (showIndicator = false) => {
    ...
    try {
      ...
      if (error) {
        ...
        // Fallback to seeded initial tools so directory remains fully operational
        ...
        const mappedTools: Tool[] = data.map((row: any) => ({
          ...
          tags: Array.isArray(row.tags) ? row.tags : (typeof row.tags === 'string' && row.tags.trim() ? row.tags.split(',').map((s: string) => s.trim()) : []),
  ...
  }, [tools, searchTerm, selectedCategory, selectedBadge, sortBy, showBookmarksOnly, bookmarkedIds]);
          ...
          <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="text-xs font-semibold px-2.5 py-1.5 rounded-lg bg-zinc-900 hover:bg-zinc-800 text-zinc-300 border border-zinc-800 flex items-center gap-1.5 transition-colors"
        ...
        <main className="max-w-6xl mx-auto px-4 py-12">
          ...
          <section className="text-center max-w-2xl mx-auto mb-12">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-semibold mb-4">
              ...
              <span>{tools.length} Tools Connected via Supabase</span>
            </div>
            ...
            <p className="text-zinc-400 text-base leading-relaxed">
              A live repository of top-tier AI applications, developer SDKs, and workflow tools stored in a Supabase backend.
            </p>
          </section>

          {/* Database Banner Note */}
          {dbError && (
            <div className="mb-8 max-w-3xl mx-auto p-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl text-xs text-amber-300 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => setIsConfigOpen(true)}
                className="px-3 py-1 bg-amber-500/20 hover:bg-amber-500/30 text-amber-200 font-semibold rounded-lg shrink-0 transition-colors"
              >
                Configure Keys
              </button>
            </div>
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto no-scrollbar justify-start sm:justify-end">
                ...
                <div className="flex items-center gap-1 bg-zinc-900 p-1 border border-zinc-800 rounded-xl shrink-0">
                  {['All', 'Free', 'Freemium', 'Paid'].map(pricing => (
                    <button
                      key={pricing}
                      onClick={() => setSelectedBadge(pricing)}
                      className={`text-[11px] font-semibold px-2.5 py-1 rounded-lg transition-all ${
                        selectedBadge === pricing
                ...
                <div className="relative shrink-0">
                  ...
                  >
                    <option value="upvotes">Sort: Top Upvoted</option>
            ...
            <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {filteredTools.length > 0 ? (
                ...
                <div className="col-span-full py-16 text-center border border-dashed border-zinc-800 rounded-2xl bg-zinc-900/20 px-4">
                  ...
                  <h3 className="text-zinc-200 font-bold text-base mb-1">No matching tools discovered</h3>
                  <p className="text-zinc-500 text-sm max-w-md mx-auto mb-6">
                    Try resetting your search query or submit a new tool directly to the Supabase database.
                  </p>
          ...
          <section id="submit" className="mt-20 p-8 border border-zinc-900 bg-gradient-to-b from-zinc-900/50 to-zinc-950/80 rounded-2xl max-w-xl mx-auto text-center relative overflow-hidden shadow-xl">
            ...
            <h2 className="text-xl font-bold mb-2 text-white">Have a Tool to Feature?</h2>
            <p className="text-zinc-400 text-xs leading-relaxed mb-6">
              Submit a GitHub link or production landing page to insert into our live Supabase <code className="text-blue-400">tools</code> table.
            </p>
            <button 
              onClick={() => setIsSubmitOpen(true)} 
        ...
        <div className="max-w-6xl mx-auto px-4 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs text-zinc-500">
          <div className="flex items-center gap-2">
            ...
            <span className="flex items-center gap-1">
              ...
              <span>{tools.length} Tools in Supabase ({totalFreeCount} Free)</span>
            </span>
          </div>
